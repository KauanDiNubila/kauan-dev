...
type CarouselMedia = {
  src: string;
  alt: string;
  /** 'image' (default) or 'video' — video renders muted/autoplay/loop, no controls. */
  type?: "image" | "video";
  poster?: string;
};
...
const Carousel_002 = ({
  ...
  autoplay?: boolean;
  spaceBetween?: number;
  /** CSS width/height of the swiper itself (e.g. "100%", "320px"). */
  width?: string;
  height?: string;
}) => {
  const css = `
  .Carousal_002 {
    padding-bottom: ${showPagination ? "50px" : "0px"} !important;
  }
  `;
  return (
    <motion.div
